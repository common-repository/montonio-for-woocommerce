...
class Montonio_Shipping {
    ...
    protected function register_hooks() {
        ...
        add_filter( 'woocommerce_order_actions', array( $this, 'add_print_labels_action_single' ), 10, 2 );
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'add_shipping_scripts'), 20 );
    ...
    public function update_checkout_pickup_point( $orderId ) {
        ...
        if (!$order) {
            WC_Montonio_Logger::log(sprintf('UpdateCheckoutPickupPoint: Order with id %s not found', $orderId));
            return;
        }
        ...
        if (method_exists($order, 'set_shipping')) {
            ...
            $order->update_meta_data( '_shipping_address_1', $chosenPickupPoint->name );
            $order->update_meta_data( '_shipping_address_2', '' );
            $order->update_meta_data( '_shipping_city', $chosenPickupPoint->locality );
            $order->update_meta_data( '_shipping_postcode', '' );
        }
    ...
    public function add_print_labels_action( $actions ) {
        ...
        wp_localize_script( 
            'montonio-print-labels', 
            'montonioPrintLabelsData', 
            array(
    ...
    public function add_print_labels_action_single( $actions, $order = null ) {
        /**
         * @since 7.0.0 - In old versions of WooCommerce, $order is not passed as a parameter
         */
        if (!$order) {
        ...
        wp_localize_script( 
            ...
            'montonioPrintLabelsData', 
            array(
                'orderId' => isset( $order ) ? $order->get_id() : null,
                'createLabelsUrl' => add_query_arg( 'wc-api', 'montonio_shipping_create_labels', trailingslashit( get_home_url() ) ),
            ) 
        );
    ...
    public function edit_shipping_method_label( $label, $rate ) {
        ...
        // Montonio_Itella_Parcel_Machines has been renamed to Montonio_Smartpost_Parcel_Machines
        // Montonio_Itella_Post_Offices has been renamed to Montonio_Smartpost_Post_Offices
        // Montonio_Itella_Courier has been renamed to Montonio_Smartpost_Courier
        // Montonio_Venipak_Post_Offices has been renamed to Montonio_Venipak_Parcel_Shops
        if ( $class_name === 'Montonio_Itella_Parcel_Machines' ) {
            $class_name = 'Montonio_Smartpost_Parcel_Machines';
        } else if ( $class_name === 'Montonio_Itella_Post_Offices' ) {
            $class_name = 'Montonio_Smartpost_Post_Offices';
        } else if ( $class_name === 'Montonio_Itella_Courier' ) {
            $class_name = 'Montonio_Smartpost_Courier';
        } else if ( $class_name === 'Montonio_Venipak_Post_Offices' ) {
            $class_name = 'Montonio_Venipak_Parcel_Shops';
        } 
