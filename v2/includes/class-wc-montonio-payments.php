    ...
    public function sync_banks_ota( $status_report ) {
        try {
            ...
            if ( isset( $bank_list ) ) {
                ...
                $settings['bank_list_fetch_datetime'] = time();
                update_option( 'woocommerce_wc_montonio_payments_settings', $settings );
                MontonioHelper::append_to_status_report( $status_report, 'success', 'Bank list sync successful' );
            } else {
                MontonioHelper::append_to_status_report( $status_report, 'error', 'Bank list sync failed' );
            }
        } catch ( Exception $e ) {
            MontonioHelper::append_to_status_report( $status_report, 'error', $e->getMessage() );
        }
